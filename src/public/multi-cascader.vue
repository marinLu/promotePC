<template>
  <div class='cascader-wrapper'>
    <el-popover placement='bottom-start' trigger='manual' popper-class='cascader-popper' v-model='showPopover'>
      <div slot='reference'>
        <el-select multiple @change='selectChange' @click.native="sysSelect" v-model='selectedLabels' :placeholder='placeholder' style='width: 100%;' popper-class='hide-popper' @focus='handleFocus' :disabled='disabled'></el-select>
      </div>
      <div class='cascader-menu-wrapper'>
        <ul class='el-cascader-menu cascader-menu' v-for='(cas, index) in casTree' :key='index'>
          <li :class="{
            ...
            }" @click='spreadNext(item.children, index)' v-for='(item, itemIdx) in cas' :key='itemIdx'>
            <el-checkbox :disabled='item.disabled' v-model='item.checked' @change='checked => { checkedChange(item, index, checked) }'></el-checkbox>
...
export default {
  ...
  mounted () {
    ...
    window.addEventListener('click', function () {
      if (document.getElementsByClassName('el-popover')[0]) {
  ...
  methods: {
    ...
    checkedChange (item, index, checked) {
      ...
      if (index === 3) { // 选择街道
        if (checked) {
          ...
          let labelTemp = this.selectedItems.map(temp => {
            return temp.label
          })
          this.selectedItems = this.selectedItems.filter(selectedItem => {
            labelTemp.map(labelTempItem => {
              return selectedItem.label !== labelTempItem
            })
          })
    ...
    setPopperWidth () {
      ...
      document.getElementsByClassName('cascader-popper')[0].style.width = width + 'px'
    },
    handleFocus () {
